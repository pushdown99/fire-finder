...
SPI spi(D11, D12, D13);
MCP3008 mcp3008(&spi, D10);
//BufferedSerial ser(PC_4, PC_5, 115200);
Serial uno(PC_4, PC_5); //TX, RX

DigitalOut dirPin(D0);
DigitalOut stepPin(D1);
...
void threadAudioTask ()
{
    spi.frequency(SPI_CLOCK);
    
    int    vol[NUM_OF_CHANNEL][NUM_OF_SAMPLES];
    double fft[NUM_OF_CHANNEL][NUM_OF_SAMPLES];
    int    dat[NUM_OF_CHANNEL][NUM_OF_SAMPLES];
    int i, j;
    double max_peak;
    int freq[NUM_OF_CHANNEL];
    int peak[NUM_OF_CHANNEL];
    ...
    for (;;) {
        for (i = 0; i < NUM_OF_CHANNEL; i++) {
            ...
            for(j = 0; j < NUM_OF_SAMPLES; j++) {
                vol[i][j]  = mcp3008.read_input_u16(i);
                max_peak   = (int)(max_peak > vol[i][j])? max_peak: vol[i][j];
                fft[i][j]  = ((double)vol[i][j])/1024.0f;
                dat[i][j]  = 0;
            }
            peak[i] = max_peak;
        } 
        for (i = 0; i < NUM_OF_CHANNEL; i++) {
            cdft(NUM_OF_SAMPLES,  1, fft[i]);       
            cdft(NUM_OF_SAMPLES, -1, fft[i]);
            for(j = 0; j < NUM_OF_SAMPLES; j++) {
                dat[i][(int)(fft[i][j] * 2)] += 1;
            }
            for(j = 0; j < NUM_OF_SAMPLES; j++) {
                if(dat[i][j] > 0) freq[i] += 1;
            }
        }       

        for (i = 0; i < NUM_OF_CHANNEL; i++) {
            pc.printf("%4d %4d ", freq[i], peak[i]);
        } 
        pc.printf("\n");  
#ifdef DEBUG
        for (i = 0; i < NUM_OF_CHANNEL; i++) {
            for(j = 0; j < NUM_OF_SAMPLES; j++) {
                pc.printf("%4d ", vol[i][j]);
            }
            pc.printf("\n");
        } 
        pc.printf("\n");  
        ...
        for (i = 0; i < NUM_OF_CHANNEL; i++) {
            for(j = 0; j < NUM_OF_SAMPLES; j++) {
                pc.printf("%4d ", dat[i][j]);
            }
            pc.printf("\n");
        } 
        pc.printf("\n");  
#endif
        if(peak[0] >= 700 & (peak[0] - peak[1]) >= 200) {
            printf("\n%4d <--- %4d \n\n", peak[0], peak[1]);
            uno.putc('0');wait(0.1);
        }
        else if((peak[1] - peak[0]) >= 200 & peak[1] >= 700) {
            printf("\n%4d ---> %4d \n\n", peak[0], peak[1]);
            uno.putc('1');wait(0.1);
        }
        else if(freq[0] >= 14 & (freq[0] - freq[1]) >= 7) {
            printf("\n%4d <--- %4d \n\n", freq[0], freq[1]);
            uno.putc('0');wait(0.1);
        }
        else if((freq[1] - freq[0]) >= 7 & freq[1] >= 14) {
            printf("\n%4d ---> %4d \n\n", freq[0], freq[1]);
            uno.putc('1'); wait(0.1);
        }
        wait(0.1);
...
int main()
{
    ...
    pc.printf("\n");
    pc.printf("************************************************************\n");
    pc.printf("***   STM32 IoT Discovery kit for STM32L475 MCU          ***\n");
    pc.printf("***         WIFI Web Server demonstration                ***\n\n");
    pc.printf("*** Copy the IP address on another device connected      ***\n");
    pc.printf("*** to the wifi network                                  ***\n");
    pc.printf("*** Read the temperature and update the LED status       ***\n");
    pc.printf("************************************************************\n");
    
   
    threadAudio.start(threadAudioTask);
    while(1) {
        wait(1);
    }
